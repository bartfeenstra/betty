...
class TestProxyAttr:
    def test_field__without_field(self) -> None:
        proxied = OwnerAttr(DataDefinition(None, label="-"))
        assert ProxyAttr(proxied=proxied).field is proxied.field

    def test_field__with_field(self) -> None:
        field = FieldDefinition(DataDefinition(None, label="-"))
        assert (
            ProxyAttr(field, proxied=OwnerAttr(DataDefinition(None, label="-"))).field
            is field
        )
