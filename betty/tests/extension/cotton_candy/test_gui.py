...
class TestColorConfigurationWidget:
    async def test_configure(self, betty_qtbot: BettyQtBot, mocker: MockerFixture) -> None:
        ...
        sut = _ColorConfigurationWidget(betty_qtbot.app, color_configuration, configured_hex_value)
    ...
    async def test_reset(self, betty_qtbot: BettyQtBot) -> None:
        ...
        color_configuration = _ColorConfiguration(hex_value='#000000')
        sut = _ColorConfigurationWidget(betty_qtbot.app, color_configuration, default_hex_value)
        betty_qtbot.qtbot.addWidget(sut)
        sut.show()
        sut._reset.click()
...
class TestCottonCandyGuiWidget:
    async def test_add_featured_entities(self, betty_qtbot: BettyQtBot) -> None:
        ...
        assert betty_qtbot.app.extensions[CottonCandy].configuration.featured_entities[0].entity_id == entity_id
    ...
    async def test_change_featured_entities(self, betty_qtbot: BettyQtBot) -> None:
        ...
        assert betty_qtbot.app.extensions[CottonCandy].configuration.featured_entities[1].entity_id == entity_id
    ...
    async def test_remove_featured_entities(self, betty_qtbot: BettyQtBot) -> None:
        ...
        assert entity_reference_1 in betty_qtbot.app.extensions[CottonCandy].configuration.featured_entities
        assert entity_reference_2 not in betty_qtbot.app.extensions[CottonCandy].configuration.featured_entities
        assert entity_reference_3 in betty_qtbot.app.extensions[CottonCandy].configuration.featured_entities

    async def test_change_primary_inactive_color(self, betty_qtbot: BettyQtBot, mocker: MockerFixture) -> None:
        configured_hex_value = '#ffffff'
        betty_qtbot.app.project.configuration.extensions.enable(CottonCandy)
        sut = _CottonCandyGuiWidget(betty_qtbot.app)
        mocker.patch.object(QColorDialog, 'getColor', mocker.MagicMock(return_value=QColor.fromString(configured_hex_value)))
        sut._color_configurations_widget._color_configurations[0]._configure.click()
        assert configured_hex_value == betty_qtbot.app.extensions[CottonCandy].configuration.primary_inactive_color.hex

    async def test_change_primary_active_color(self, betty_qtbot: BettyQtBot, mocker: MockerFixture) -> None:
        configured_hex_value = '#ffffff'
        betty_qtbot.app.project.configuration.extensions.enable(CottonCandy)
        sut = _CottonCandyGuiWidget(betty_qtbot.app)
        mocker.patch.object(QColorDialog, 'getColor', mocker.MagicMock(return_value=QColor.fromString(configured_hex_value)))
        sut._color_configurations_widget._color_configurations[1]._configure.click()
        assert configured_hex_value == betty_qtbot.app.extensions[CottonCandy].configuration.primary_active_color.hex

    async def test_change_link_inactive_color(self, betty_qtbot: BettyQtBot, mocker: MockerFixture) -> None:
        configured_hex_value = '#ffffff'
        betty_qtbot.app.project.configuration.extensions.enable(CottonCandy)
        sut = _CottonCandyGuiWidget(betty_qtbot.app)
        mocker.patch.object(QColorDialog, 'getColor', mocker.MagicMock(return_value=QColor.fromString(configured_hex_value)))
        sut._color_configurations_widget._color_configurations[2]._configure.click()
        assert configured_hex_value == betty_qtbot.app.extensions[CottonCandy].configuration.link_inactive_color.hex

    async def test_change_link_active_color(self, betty_qtbot: BettyQtBot, mocker: MockerFixture) -> None:
        configured_hex_value = '#ffffff'
        betty_qtbot.app.project.configuration.extensions.enable(CottonCandy)
        sut = _CottonCandyGuiWidget(betty_qtbot.app)
        mocker.patch.object(QColorDialog, 'getColor', mocker.MagicMock(return_value=QColor.fromString(configured_hex_value)))
        sut._color_configurations_widget._color_configurations[3]._configure.click()
        assert configured_hex_value == betty_qtbot.app.extensions[CottonCandy].configuration.link_active_color.hex
