...
class TestTrees:
    async def test_generate(self) -> None:
        async with (App.new_temporary() as app, app):
            ...
            await generate(app)
        async with aiofiles.open(app.project.configuration.www_directory_path / 'js' / 'betty.extension.Trees.js', encoding='utf-8') as f:
            betty_js = await f.read()
        assert Trees.name() in betty_js
        async with aiofiles.open(app.project.configuration.www_directory_path / 'css' / 'betty.extension.Trees.css', encoding='utf-8') as f:
