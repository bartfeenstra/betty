...
class AppTest(TestCase):
    ...
    async def test_extensions_with_one_extension_with_single_chained_dependency(self) -> None:
        configuration = Configuration(**self._MINIMAL_CONFIGURATION_ARGS)
        configuration.extensions[DependsOnNonConfigurableExtensionExtensionExtension] = ExtensionConfiguration(DependsOnNonConfigurableExtensionExtensionExtension)
        async with App(configuration) as sut:
            carrier = []
    ...
    async def test_extensions_with_multiple_extensions_with_duplicate_dependencies(self) -> None:
        configuration = Configuration(**self._MINIMAL_CONFIGURATION_ARGS)
        configuration.extensions[DependsOnNonConfigurableExtensionExtension] = ExtensionConfiguration(DependsOnNonConfigurableExtensionExtension)
        configuration.extensions[AlsoDependsOnNonConfigurableExtensionExtension] = ExtensionConfiguration(AlsoDependsOnNonConfigurableExtensionExtension)
        async with App(configuration) as sut:
            carrier = []
            await sut.dispatcher.dispatch(Tracker, 'track')(carrier)
            self.assertEquals(3, len(carrier))
    ...
    async def test_extensions_with_multiple_extensions_with_cyclic_dependencies(self) -> None:
        configuration = Configuration(**self._MINIMAL_CONFIGURATION_ARGS)
        configuration.extensions[CyclicDependencyOneExtension] = ExtensionConfiguration(CyclicDependencyOneExtension)
    ...
    async def test_extensions_with_comes_before_with_other_extension(self) -> None:
        configuration = Configuration(**self._MINIMAL_CONFIGURATION_ARGS)
        configuration.extensions[NonConfigurableExtension] = ExtensionConfiguration(NonConfigurableExtension)
        configuration.extensions[ComesBeforeNonConfigurableExtensionExtension] = ExtensionConfiguration(ComesBeforeNonConfigurableExtensionExtension)
        async with App(configuration) as sut:
            carrier = []
            await sut.dispatcher.dispatch(Tracker, 'track')(carrier)
    ...
    async def test_extensions_with_comes_after_with_other_extension(self) -> None:
        configuration = Configuration(**self._MINIMAL_CONFIGURATION_ARGS)
        configuration.extensions[ComesAfterNonConfigurableExtensionExtension] = ExtensionConfiguration(ComesAfterNonConfigurableExtensionExtension)
        configuration.extensions[NonConfigurableExtension] = ExtensionConfiguration(NonConfigurableExtension)
        async with App(configuration) as sut:
            carrier = []
            await sut.dispatcher.dispatch(Tracker, 'track')(carrier)
    ...
    async def test_extensions_addition_to_configuration(self) -> None:
        ...
        async with App(configuration) as sut:
            ...
            sut.extensions
            configuration.extensions[NonConfigurableExtension] = ExtensionConfiguration(NonConfigurableExtension)
            self.assertIn(NonConfigurableExtension, sut.extensions)
