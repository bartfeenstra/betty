...
from typing import Callable, Dict, Any, Iterable, Optional, Union, AsyncIterable, Type, Iterator
...
from jinja2 import contextfilter, escape, FileSystemLoader, Template, Environment, select_autoescape, StrictUndefined
from jinja2.asyncsupport import auto_await
from jinja2.filters import prepare_map, evalcontextfilter, make_attrgetter
...
class BettyTemplate(Template):
    def render(self, *args, **kwargs):
        ...
        self.build_vars(vars)
        return Template.render(self, vars)

    async def render_async(self, *args, **kwargs):
        vars = dict(*args, **kwargs)
        self.build_vars(vars)
        return await Template.render_async(self, vars)

    @property
    def build_vars(self) -> Callable[[Dict[str, Any]], None]:
        try:
            return self._vars_builder
        except AttributeError:
            return self._passthrough_vars

    @build_vars.setter
    def build_vars(self, builder: Callable[[Dict[str, Any]], None]):
        self._vars_builder = builder

    def _passthrough_vars(self, vars: Dict[str, Any]) -> None:
        pass
...
class BettyEnvironment(Environment):
    ...
    def get_template(self, name, parent=None, globals=None) -> Template:
        ...
        with suppress(KeyError):
            template.build_vars = self.template_vars_builders[name]
        return template
