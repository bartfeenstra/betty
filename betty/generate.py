...
async def generate(site: Site) -> None:
    ...
    for locale_configuration in site.configuration.locales.values():
        await _generate_localized(site, locale_configuration)
...
async def _generate_localized(site: Site, locale_configuration: LocaleConfiguration) -> None:
    ...
    async with site.with_locale(locale_configuration.locale) as site:
        ...
        await _generate_resource_type(www_directory_path, site.ancestry.places.values(), 'place', site, locale_configuration.locale, environment)
        await _generate_resource_type(www_directory_path, site.ancestry.events.values(), 'event', site, locale_configuration.locale, environment)
        await _generate_resource_type(www_directory_path, site.ancestry.citations.values(), 'citation', site, locale_configuration.locale, environment)
        await _generate_resource_type(www_directory_path, site.ancestry.sources.values(), 'source', site, locale_configuration.locale, environment)
        await _generate_openapi(www_directory_path, site)
...
async def _generate_resource_type(www_directory_path: str, resources: List[Any], resource_type_name: str, site: Site,
    ...
    queue = asyncio.Queue()
    queue.put_nowait(_generate_resource_type_list_html(www_directory_path, resources, resource_type_name, environment))
    queue.put_nowait(_generate_resource_type_list_json(www_directory_path, resources, resource_type_name, site))
    for resource in resources:
        queue.put_nowait(_generate_resource(www_directory_path, resource, site, locale, environment))
    await asyncio.gather(
        *[asyncio.create_task(_worker(queue)) for _ in range(99)],
...
async def _generate_resource_type_list_html(www_directory_path: str, resources: Iterable[Any], resource_type_name: str,
    ...
    with suppress(TemplateNotFound):
        ...
        async with _create_html_resource(resource_type_path) as f:
            await f.write(await template.render_async({
...
async def _generate_resource(www_directory_path: str, resource: Union[Resource, Identifiable], site: Site, locale: str, environment: Environment) -> None:
...
async def _generate_resource_html(www_directory_path: str, resource: Union[Resource, Identifiable], environment: Environment) -> None:
    ...
    async with _create_html_resource(resource_path) as f:
        await f.write(await environment.get_template('page/%s.html.j2' % resource.resource_type_name).render_async({
...
async def _generate_resource_json(www_directory_path: str, resource: Union[Resource, Identifiable], site: Site, locale: str) -> None:
    ...
    async with _create_json_resource(resource_path) as f:
        await f.write(dumps(resource, cls=JSONEncoder.get_factory(site, locale)))
...
async def _generate_openapi(www_directory_path: str, site: Site) -> None:
    async with aiofiles.open(join(www_directory_path, 'api', 'index.json'), 'w') as f:
        await f.write(dumps(build_specification(site)))
