    ...
    with suppress(TemplateNotFound):
        ...
        async with _create_html_resource(entity_type_path) as f:
            await f.write(await template.render_async({
                ...
                'entity_tyfepe_name': entity_type_name,
                'entities': entities,
            }))


async def _generate_entity_type_list_json(www_directory_path: str, entities: Iterable[Any], entity_type_name: str, site: Site) -> None:
    entity_type_path = os.path.join(www_directory_path, entity_type_name)
...
async def _generate_entity(www_directory_path: str, entity: Any, entity_type_name: str, site: Site, locale: str, environment: Environment) -> None:
    ...
    await _generate_entity_json(www_directory_path, entity,
...
async def _generate_entity_html(www_directory_path: str, entity: Any, entity_type_name: str, environment: Environment) -> None:
    ...
    async with _create_html_resource(entity_path) as f:
        await f.write(await environment.get_template('page/%s.html.j2' % entity_type_name).render_async({
...
async def _generate_entity_json(www_directory_path: str, entity: Any, entity_type_name: str, site: Site, locale: str) -> None:
    entity_path = os.path.join(www_directory_path, entity_type_name, entity.id)
    async with _create_json_resource(entity_path) as f:
        await f.write(json.dumps(entity, cls=JSONEncoder.get_factory(site, locale)))
...
async def _generate_openapi(www_directory_path: str, site: Site) -> None:
    async with aiofiles.open(join(www_directory_path, 'api', 'index.json'), 'w') as f:
        await f.write(json.dumps(build_specification(site)))
