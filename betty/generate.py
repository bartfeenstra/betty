...
from betty.task import TaskBatch, TaskP
...
async def generate(app: App) -> None:
    ...
    # @todo Now, how do we ensure that Jinja2 filters have access to this task batch?
    # @todo Because at any time, App may be serving multiple batches besides the manager's own
    # @todo
    pickled_app = dill.dumps(app)
    localized_process_batches: dict[str | None, TaskBatch[GenerationTaskBatchContext]] = {}
    ...
    # @todo Because the ??? of batches is not done until all batches are done.
