...
class AttrTestBase(Generic[_InstanceT, _ValueT]):
    ...
    def test_new_attr(self) -> None:
        """
        Tests :py:meth:`betty.attr.Attr.new_attr` implementations.
        """
...
class SettableAttrTestBase(
    Generic[_InstanceT, _ValueT, _SetT], AttrTestBase[_InstanceT, _ValueT]
):
    """
    A base class for testing :py:class:`betty.attr.SettableAttr` implementations.
    """
    ...
    def get_mutable_instances(
        ...
        :return: A 2-tuple with the instances, and the name of the attribute containing the :py:class:`betty.attr.SettableAttr` under test.
        """
        raise NotImplementedError(repr(self))
    ...
    def test_set_attr(self) -> None:
        """
        Tests :py:meth:`betty.attr.SettableAttr.set_attr` implementations.
        """
        instances, attr_name = self.get_mutable_instances()
        for instance, set_values in instances:
            for set_value in set_values:
...
class DeletableAttrTestBase(
    Generic[_InstanceT, _ValueT, _SetT],
    SettableAttrTestBase[_InstanceT, _ValueT, _SetT],
):
    """
    A base class for testing :py:class:`betty.attr.DeletableAttr` implementations.
    """
    ...
    def test_del_attr(self) -> None:
        """
        Tests :py:meth:`betty.attr.DeletableAttr.del_attr` implementations.
        """
        instances, attr_name = self.get_mutable_instances()
        for instance, set_values in instances:
            # Test deleting any default value.
