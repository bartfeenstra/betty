...
from betty.locale import Localized, negotiate_locale, to_locale, get_data, LocaleNotFoundError
    ...
    async def _populate_entity(self, entity: Entity, locales: Set[str]) -> None:
        ...
        for entry_locale, entry_name in list(entry_links):
            ...
            if len(entry_translations) == 0:
                continue
            entry_translation_locales = set(filter_suppress(get_data, LocaleNotFoundError, entry_translations.keys()))
            for locale in locales.difference({entry_locale}):
                added_entry_locale_data = negotiate_locale(locale, entry_translation_locales)
    ...
    async def populate_link(self, link: Link, entry_locale: str, entry: Optional[Entry] = None) -> None:
        ...
        if link.description is None:
            ...
            with suppress(ValueError):
                link.description = self._app.translations.get_negotiated(link.locale)._('Read more on Wikipedia.')
